...
// Normalizes members coming from FormData — could arrive as "members[]",
// "members", a single string, or a JSON-stringified array depending on
// how the client sent it, so we accept all of those shapes here.
const normalizeMembers = (body) => {
  let raw = body["members[]"] ?? body.members ?? [];
...
exports.createGroup = async (req, res) => {
  try {
    ...
    const members = normalizeMembers(req.body);
    ...
    const groupData = {
      ...
      members,
      createdBy: req.user?._id || members[0],
    };

    if (req.file?.path) groupData.groupImage = req.file.path;

    const group = await Group.create(groupData);

    res.status(201).json({ group, msg: "Group created successfully" });
  } catch (error) {
    console.log(error);
