...
const pinPrivateMessage = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const chat = await PrivateChat.findById(msg.chatId);
    if (!chat || !chat.members.map((m) => m.toString()).includes(req.user.id)) {
      return res.status(403).json({
        success: false,
    ...
    if (io) {
      ...
      let target = io.to(msg.chatId.toString());
      if (receiverId) target = target.to(`user_${receiverId}`);

      target.emit("privateMessagePinned", {
        message: populated,
...
const deleteChat = async (req, res) => {
  try {
    ...
    if (!chat.members.map((m) => m.toString()).includes(myId)) {
      return res.status(403).json({ message: "Not authorized to delete this chat" });
    }

    await PrivateMessage.deleteMany({ chatId });
    await Call.deleteMany({ chatId });
    await PrivateChat.findByIdAndDelete(chatId);

    const io = req.app.get("io");
    if (io) {
      io.to(chatId.toString()).emit("privateChatDeleted", { chatId });
      chat.members.forEach((m) => {
        io.to(`user_${m.toString()}`).emit("privateChatDeleted", { chatId });
      });
    }
