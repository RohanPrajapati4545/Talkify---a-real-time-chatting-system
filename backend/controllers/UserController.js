...
const updateProfile = async (req, res) => {
  try {
    ...
    if (req.body.phone || req.body.contact) {
      user.contact = req.body.phone || req.body.contact;
    }
...
const togglePinChat = async (req, res) => {
  try {
    ...
    const existingIndex = user.pinnedChats.findIndex(
      (p) => p.chatId.toString() === chatId.toString() && p.chatType === chatType
    );

    let isPinned = false;
    if (existingIndex > -1) {
      user.pinnedChats.splice(existingIndex, 1);
      isPinned = false;
    } else {
      user.pinnedChats.push({ chatId: chatId.toString(), chatType, pinnedAt: new Date() });
      isPinned = true;
    }
