...
const pinGroupMessage = async (req, res) => {
  try {
    ...
    const group = await GroupSchema.findById(msg.groupId);
    if (!group) {
      return res.status(404).json({ success: false, message: "Group not found" });
    }

    // Check if user is group creator or system admin
    const isCreator = group.createdBy.toString() === req.user.id;
    const isSystemAdmin = req.user.role === "admin";
    if (!isCreator && !isSystemAdmin) {
      return res.status(403).json({
        success: false,
        message: "Only group admin or system admin can pin messages",
      });
    }
    ...
    const io = req.app.get("io");
    if (io) {
      io.to(msg.groupId.toString()).emit("groupMessagePinned", {
    ...
    res.status(200).json({
      success: true,
      message: msg.isPinned ? "Message pinned successfully" : "Message unpinned successfully",
