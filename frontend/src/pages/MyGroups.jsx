...
useEffect(() => {
  ...
  const openPrivateChat = async (targetUser, pushHistory = true) => {
    ...
    try {
      ...
      await getPrivateCallLogs(res.data._id);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      socket.off("privateMessagePinned");
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    try {
      ...
      const savedGroupStr = localStorage.getItem("talkify_active_chat_group");
      if (savedUserStr) {
...
useEffect(() => {
  ...
  const isGroupAdminOrSystemAdmin = useMemo(() => {
    if (!selectedGroup) return false;
    const isCreator =
      (selectedGroup.createdBy?._id || selectedGroup.createdBy)?.toString() ===
      user?._id?.toString();
    const isSystemAdmin = user?.role === "admin";
    return Boolean(isCreator || isSystemAdmin);
  }, [selectedGroup, user]);

  const activePinnedGroupMsg = useMemo(() => {
    if (!messages || messages.length === 0) return null;
    return messages.slice().reverse().find((m) => m.isPinned && !m.isDeleted) || null;
  }, [messages]);

  const activePinnedPrivateMsg = useMemo(() => {
    if (!privateMessages || privateMessages.length === 0) return null;
    return privateMessages.slice().reverse().find((m) => m.isPinned && !m.isDeleted) || null;
  }, [privateMessages]);
  ...
  const handlePinGroupMessage = async (msg) => {
    try {
      setShowMsgMenu(null);
      const res = await axios.put(
        `${process.env.REACT_APP_API_URL}/api/user/pin-message/${msg._id}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.data.success) {
        toast.success(res.data.message);
        setMessages((prev) =>
          prev.map((m) => (m._id === msg._id ? res.data.messageData : m))
        );
      }
    } catch (err) {
      console.error("Pin message error:", err);
      toast.error(err.response?.data?.message || "Failed to update pinned state");
    }
  };

  const handlePinPrivateMessage = async (msg) => {
    try {
      setShowMsgMenu(null);
      const res = await axios.put(
        `${process.env.REACT_APP_API_URL}/api/private/pin-message/${msg._id}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.data.success) {
      ...
      console.error("Pin private message error:", err);
      toast.error(err.response?.data?.message || "Failed to update pinned state");
    }
  };
  ...
  const scrollToMessage = (msgId) => {
    ...
    if (el) {
      el.scrollIntoView({ behavior: "smooth", block: "center" });
      el.classList.add("cv-msg-highlight-pulse");
      setTimeout(() => {
        el.classList.remove("cv-msg-highlight-pulse");
      }, 2200);
    } else {
      toast.info("Pinned message is in earlier chat history. Scroll up to load older messages.");
    }
  };
    ...
    <div className="  cv-page-container">
      ...
      <div className="cv-shell">
        ...
        <div className="row g-0 cv-grid">
          ...
          <div
            className={`col-12 col-md-4 p-0 ${chatOpen ? "d-none d-md-flex" : "d-flex"
              }`}
          >
          <SideBar
  activeTab={activeTab}
...
/>
          ...
          <div
            className={`col-12 col-md-8 p-0 ${chatOpen ? "d-flex" : "d-none d-md-flex"
              }`}
          >
              ...
              {selectedUser ? (
                ...
                showUserInfo ? (
                  ...
                  <>
                        ...
                        <div>
                          ...
                          <div className="cv-thread-status">
                            {isPrivateUserTyping ? (
                              ...
                              <span className="cv-online-status">
                                <span className="cv-status-dot-inline online"></span> Online
                              </span>
                            ) : (
                              <span className="cv-offline-status">
                                <span className="cv-status-dot-inline offline"></span> Offline
                              </span>
                            )}
                          </div>
                        </div>
                    ...
                    {activePinnedPrivateMsg && (
                      <div
                        className="cv-pinned-banner"
                        onClick={() => scrollToMessage(activePinnedPrivateMsg._id)}
                        title="Click to view pinned message"
                      >
                        <div className="cv-pinned-icon-box">
                          <i className="fa-solid fa-thumbtack"></i>
                        </div>
                        <div className="cv-pinned-info">
                          <div className="cv-pinned-title">
                            Pinned Message · <span className="cv-pinned-sender">{activePinnedPrivateMsg.sender?.name || "User"}</span>
                          </div>
                          <div className="cv-pinned-preview">
                            {activePinnedPrivateMsg.message ? (
                              activePinnedPrivateMsg.message
                            ) : activePinnedPrivateMsg.mediaType === "image" ? (
                              "📷 Photo"
                            ) : activePinnedPrivateMsg.mediaType === "audio" ? (
                              "🎤 Voice Message"
                            ) : (
                              "🎥 Video"
                            )}
                        ...
                        <button
                          type="button"
                          className="cv-pinned-unpin-btn"
                          onClick={(e) => {
                            e.stopPropagation();
                            handlePinPrivateMessage(activePinnedPrivateMsg);
                          }}
                          title="Unpin message"
                        >
                                ...
                                <div className={`cv-bubble ${side}`}>
                                  ...
                                  <div className="cv-stamp">
                                    {msg.isPinned && !msg.isDeleted && (
                                      <span className="cv-msg-pinned-tag" title="Pinned message">
                                        <i className="fa-solid fa-thumbtack"></i>
                                      </span>
                                    )}
    ...
    <p>Connect · Converse · Collaborate</p>
                ...
                <>
                  ...
                  {showMsgSearch && renderMsgSearchBar()}
                    ...
                    {activePinnedGroupMsg && (
                      <div
                        className="cv-pinned-banner"
                        onClick={() => scrollToMessage(activePinnedGroupMsg._id)}
                        title="Click to view pinned message"
                      >
                        <div className="cv-pinned-icon-box">
                          <i className="fa-solid fa-thumbtack"></i>
                        ...
                        <div className="cv-pinned-info">
                          <div className="cv-pinned-title">
                            Pinned Message · <span className="cv-pinned-sender">{activePinnedGroupMsg.sender?.name || "Member"}</span>
                          </div>
                          <div className="cv-pinned-preview">
                            {activePinnedGroupMsg.message ? (
                              activePinnedGroupMsg.message
                            ) : activePinnedGroupMsg.mediaType === "image" ? (
                              "📷 Photo"
                            ) : activePinnedGroupMsg.mediaType === "audio" ? (
                              "🎤 Voice Message"
                            ) : (
                              "🎥 Video"
                            )}
                        ...
                        {isGroupAdminOrSystemAdmin && (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handlePinGroupMessage(activePinnedGroupMsg);
                            }}
                            title="Unpin message"
                          >
                            <i className="fa-solid fa-xmark"></i>
  ...
  {mergedGroupThread.map((item, index) => {
                          ...
                          >
                            ...
                            <div className={`cv-bubble ${side}`}>
                              ...
                              <div className="cv-stamp">
                                ...
                                <span className="cv-stamp-time">{formatTime(msg.createdAt)}</span>
                                {isMe && !msg.isDeleted && (
                                  <i className="fa-solid fa-check cv-tick-sent" title="Sent"></i>
                                )}
                              </div>
    ...
    {showCreateGroupModal && (
      ...
      >
        <div className="modal-dialog modal-dialog-centered" style={{ zIndex: 2510, pointerEvents: "auto" }}>
            ...
            <div className="modal-body">
              ...
              <div className="text-center mb-4">
                {groupImage ? (
                  ...
                  <div
                    className="cv-upload-circle"
                    style={{ cursor: "pointer", margin: "0 auto" }}
                    onClick={() => document.getElementById("createGroupFileInput")?.click()}
                  >
    ...
    {showJoinModal && (
      <div
        ...
        onClick={(e) => {
          if (e.target === e.currentTarget && !joiningGroup) {
            setShowJoinModal(false);
          }
        }}
      >
